...
| 45 | `juice_box_channels.sql` | Juice Box channels — General / Product Help / Social Media Hub. **Purely additive — expand-and-migrate, no existing object is altered.** (a) Adds `team_messages.channel TEXT NOT NULL DEFAULT 'general'` + CHECK on the three ids: a catalog-only change in PG 11+, so every existing post backfills to General with no table rewrite, and the OLD app (which never sends a channel, and never selects it) keeps working untouched. (b) Adds the partial index `idx_team_messages_live_channel_created_at ON (channel, created_at DESC) WHERE is_deleted = false`; the older live_created_at index is KEPT for cross-channel search and the old bundle's channel-less feed query. (c) Creates a NEW table `team_message_channel_reads` keyed `(salesperson_id, channel)` (CHECK on the three ids, `updated_at` trigger, RLS ENABLED with NO policy, not in `supabase_realtime` — the same posture as `team_message_reads`) and COPIES every legacy `team_message_reads` row into it as that person's **General** marker via `ON CONFLICT (salesperson_id, channel) DO NOTHING` — idempotent, and a re-run can never duplicate rows or rewind a marker the new app has since advanced. **`team_message_reads` is deliberately left exactly as it is** (same columns, same single-column unique index, same trigger, same grants) so the previously deployed bundle and any stale browser tab keep marking reads with `onConflict: "salesperson_id"`. That removes the rollout trap of the first draft, which moved that unique key and broke the live app the moment it was applied. **Either order is safe**; recommended is migration first, then deploy (the new bundle needs the column + new table). During the overlap, a General mark-read from the new app also mirrors into the legacy row, best-effort, so old clients don't sit on a stale badge. (d) Adds two `SECURITY INVOKER` RPCs — `juice_box_mark_channel_read(text, text)` and `juice_box_mark_legacy_read(text)` — that perform mark-read ATOMICALLY and MONOTONICALLY: they stamp `now()` inside Postgres and apply `last_read_at = GREATEST(existing, incoming)` in a single `INSERT … ON CONFLICT DO UPDATE`, RETURNing the persisted value. This closes a real race in the previous unconditional upsert (two requests stamping T1/T2 could reach the DB in reverse order, and the older T1 would overwrite T2, moving the marker BACKWARD and resurfacing already-read posts as unread). `EXECUTE` is REVOKED from PUBLIC/anon/authenticated and GRANTED to `service_role` only, so they are reachable solely through the server routes; SECURITY DEFINER is deliberately avoided (it would let a lower-privileged role write any salesperson's marker), and `search_path` is pinned. (e) Adds the admin **Move conversation** feature: a server-only audit table `juice_box_conversation_moves` (root message id, from/to channel, acting salesperson, message count, `moved_at`; CHECKs on both channels + `from_channel <> to_channel` + `message_count > 0`; indexed by `(root_message_id, moved_at DESC)` and `moved_at DESC`; RLS ENABLED with NO policy, not in `supabase_realtime`; no FKs, matching `team_messages.reply_to_message_id`/`salesperson_id` so history outlives the rows it describes), plus the `SECURITY INVOKER` RPC `juice_box_move_conversation(uuid, text, text)`. The RPC resolves the conversation's ROOT by walking up `reply_to_message_id` from whatever message the admin clicked, collects the entire descendant tree with a recursive CTE (nested replies included), locks the root and then every member in id order, requires ONE shared source channel, updates only the `channel` column, re-derives the tree to catch a reply inserted mid-move, and writes the audit row — all in a single transaction, so a failure rolls back both the move and the audit. It signals expected conditions with SQLSTATEs the route maps to HTTP (`P0002`→404, `JB001`/`JB002`/`JB003`→409, `JB004`/`JB005`→400) and returns `{root_message_id, from_channel, to_channel, message_count, moved_at}`. `EXECUTE` is REVOKED from PUBLIC/anon/authenticated and GRANTED to `service_role` only — the sole caller is `POST /api/team-messages/:id/move` behind `requireAdmin`, which supplies the acting admin from the verified session; the function itself cannot authorize and never trusts a client-supplied actor, source channel, or root id. No message is inserted or deleted, so no duplicates and no push notification. **RETIREMENT:** drop `team_message_reads`, `juice_box_mark_legacy_read`, and the mirror call in `/api/team-messages/reads/me` only in a SEPARATE future cleanup migration, once no pre-channels client can still be running; `juice_box_conversation_moves` is permanent moderation history and should be kept (prune by `moved_at` if it ever needs trimming). Depends on migrations #13 (`team_messages.sql`) and #14 (`team_message_reads.sql`). Additive and idempotent. |
| 46 | `gold_list.sql` | Gold List at `/gold-list`: independent AE-owned agents and activities, contact details, descriptions and DATE due dates, archive/restore, and preserved completion history. RLS enabled with no policies; only authenticated server routes access the tables. AEs read/write their own rows; admins read/filter all lists but write only their own. Composite activity/agent ownership FK and unique open-activity index enforce integrity. Triggers protect finished history/creator identity and serialize activity writes with archive changes. Names are advisory duplicate warnings (email/phone/name checks include archived records), so the draft unique-name index is removed. Re-running upgrades an already-installed draft by adding and backfilling descriptions from its legacy categories; existing categories remain for compatibility but are not exposed in forms. Client request UUIDs make create retries safe. Each activity also carries an OPTIONAL `activity_note` — the plan for that touch, written when it is scheduled and bounded by the re-addable `gold_list_activity_note_valid` CHECK. It is a **third, separate** note column: it neither replaces nor renames `outcome_note` (what happened, written on completion) or `gold_list_agents.notes` (the standing relationship note). Nullable with no backfill, so a re-run against a database already holding agents and activities just adds the column and leaves every existing row reading NULL, and the existing history trigger freezes it along with the rest of the row once an activity leaves 'scheduled'. No weekly-focus or daily-touch records are changed. **Apply/re-run before deploying this code**, including installations that already applied the original draft. Depends on `schema.sql`; transactional and idempotent. |
...
## Juice Box read markers — two tables during the channels rollout

`team_message_reads` (one row per salesperson, UNIQUE on `salesperson_id`) is
the LEGACY marker table. `team_message_channel_reads` (one row per
`(salesperson_id, channel)`) is what the channel-aware app uses. Both exist on
purpose:

- The deployed pre-channels bundle — and any stale browser tab — still upserts
  the legacy table with `onConflict: "salesperson_id"`. Migration #45 leaves
  that table's schema, unique index, trigger and grants completely untouched, so
  that keeps working while the new code is rolling out.
- Migration #45 copies every legacy row into the new table as that person's
  **General** marker, so no unread state is lost. The copy is
  `ON CONFLICT DO NOTHING`, so re-running it neither duplicates rows nor rewinds
  a marker the new app has advanced.
- **Reads are dual-sourced for General:** the copy happens when the migration
  RUNS, but the old bundle keeps running until the deploy lands and can advance
  the legacy marker after that. So General's effective marker is
  `MAX(channel-specific General, legacy)` — resolved in exactly one place,
  `fetchEffectiveChannelMarkers` in `src/lib/server/juice-box-reads.ts`, which
  both `GET /api/team-messages/unread` and `GET /api/team-messages/reads/me`
  call. Without it, a General post read on the old bundle during that window
  would come back as unread (stale NEW MESSAGES divider + a scroll into old
  history) right after the upgrade. **Product Help and Social Media Hub never
  consult the legacy marker** — it predates those channels, so folding it in
  would hide genuinely unread posts.
- **Writes** go through the monotonic RPCs, never a bare upsert:
  `juice_box_mark_channel_read` for the authoritative channel marker and — for
  General only, best-effort, in its own transaction so it cannot roll the
  primary write back — `juice_box_mark_legacy_read` for the legacy row. Both
  apply `GREATEST(existing, incoming)` inside a single statement, so **neither
  write can move a marker backwards** even if two requests arrive out of order,
  and the API returns the timestamp Postgres actually persisted. Because both
  stamp the database's own `now()`, the first General read after the upgrade
  also **self-heals** any drift between the two tables.

**Retirement (separate future migration, not now):** once no pre-channels client
can still be running, drop `team_message_reads`, delete the mirror write, and
delete `fetchLegacyMarker` + the `laterTimestamp` call in
`src/lib/server/juice-box-reads.ts` (that module then collapses to a
single-table read; its callers need no change). Nothing else in the app reads
the legacy table. Until then, treat it as load-bearing.
